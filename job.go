...
func (jbm *JobManager) FindJob(jobTime int64) ([]string, bool) {
	uniqueIds, err := redis.Strings(RedisInstance().Do("ZRANGEBYSCORE", RedisConfig.Zset, 0, jobTime))
	if err != nil {
		return nil, false
	}
	if len(uniqueIds) == 0 {
...
func (jbm *JobManager) GetJobData(uniqueId string) (*CronJob, bool) {
	jobString, err := redis.String(RedisInstance().Do("HGET", RedisConfig.Hash, uniqueId))
	if err != nil {
		return nil, false
	}
	job := &CronJob{}
...
func (jbm *JobManager) Exec(job *CronJob) {
	...
	_, err = RedisInstance().Do("ZADD", RedisConfig.Zset, job.NextRunAt, job.UUID)
	if err != nil {
	...
	_, err = RedisInstance().Do("HSET", RedisConfig.Hash, job.UUID, string(jobByte))
	if err != nil {
